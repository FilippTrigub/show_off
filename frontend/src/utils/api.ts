...
export const rephrasePost = async (content: string, tone: number): Promise<string> => {
    ...
    if (newText) {
        ...
        throw new Error("Failed to rephrase post");
    }
};

// Handles the Text-to-Speech functionality
export const generateTTS = async (content: string): Promise<string> => {
